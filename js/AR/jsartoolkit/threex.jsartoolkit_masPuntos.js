...
NyARRgbPixelReader_Canvas2D = ASKlass("NyARRgbPixelReader_Canvas2D", INyARRgbPixelReader, {
    ...
    ,
    getPixelSet: function(i_x, i_y, i_num, o_rgb) { //nop
        var idata = this.getData();
        var w = idata.width;
        var d = idata.data;
        for (var i = 0; i < i_num; i++) {
...
PerspectivePixelReader = ASKlass('PerspectivePixelReader', {
    ...
    ,
    detectDataBitsIndex: function(o_index_row, o_index_col) {
        var i;
    ...
    readDataBits: function(i_reader, o_bitbuffer) {
        ...
        for (var i = 0; i < resolution; i++) {
            ...
            if (i > 0 && i < resolution - 1) { //TroLL
                for (i2 = 1; i2 < resolution - 1; i2++) {
                    var index = i2 * 4;
                    var pixel = (pixcel_temp[index + 0] + pixcel_temp[index + 1] + pixcel_temp[index + 2] + pixcel_temp[index + 3]) / (4);
                    // 1 = bright point, 0 = dark point                
                    if (pixel == 255) {
                        o_bitbuffer.setBitByBitIndex(p, 0);
                    } else if (pixel == 0) {
                        o_bitbuffer.setBitByBitIndex(p, 1);
                    } else {
